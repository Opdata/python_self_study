...
def binary_converter(decimal_number):
    ...
    while decimal_number >0:
        ...
        result = str(binary_number) + result
...
def number_of_cases(list_data):
    ...
    for i in range(0,len(list_data)):
        for j in range(0,len(list_data)):
            new_data_list.append(str(list_data[i])+str(list_data[j]))


    result_copy= set(new_data_list)
    result = list(result_copy)
    result.sort()
    return result

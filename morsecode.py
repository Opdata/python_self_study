...
def is_help_command(user_input):
    ...
    user_input_copy = user_input.lower()
    if user_input_copy == 'h' or user_input_copy == 'help':
        result = True
    else:
        result = False
...
def is_validated_english_sentence(user_input):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    # 1. 문장에 숫자가 포함되어 있으면 ? False / 문장에 숫자가 포함되어있지않으면 True
    # 2. 문장부호를 제외하고 특수문자를 포함하면 ? False / 문장부호를 제외하고 특수문자를 포함하지않는다면 True
    # – 문장부호는 리스트 혹은 문자열로 처리 가능
    # ex. “_@#$%^&*()-+=[]{}\”\’;:|`~”
    # [“_”, “@”, “#”, “$”, “%”, “^”, “&”, “*”, “(“, “)”, “-“, “+”, “=”, “[“, “]”, “{“, “}”, ‘”‘, “‘”, “;”, “:”, “|”, “`”, “~”]
    # (“이나 ‘는 \” \’로 작성하면 됨)
    # 3. 문장부호를 제외하고 입력값이 없거나 빈칸만 입력하면 ? False
    # – 문자 안에서 문장부호를 모두 지운 후 체크
    #문장이 세가지 조건에 한가지라도 성립되지 않으면? True
    num = "0123456789"
    list_num = list(num)
    string = "@#$%^&*()_-+{}|:\"<>[];'/\`=~"     #29개
    list_string = list(num)
    specialstring = '.,?!'
    list_specialstring = list(specialstring)
    eng = "QWERTYUIOPASDFGHJKLZXCVBNM"
    numjment = False
    stringjment = False
    engjment = False
    if list_num not in user_input:
        numjment = True
        print(list_num)
    if string not in user_input:
        stringjment = True
        print(list_string)
    result = True
    return result
